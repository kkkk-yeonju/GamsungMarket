...
export default class ReactPagination extends React.Component {
  constructor(props) {
    ...
    this.showLastPagi = this.showLastPagi.bind(this);
  ...
  showLastPagi() {
    ...
    if (currentPage !== totalPages) {
      return (
        <a
          className={this.isactive(totalPages) ? "is-active" : ""}
          onClick={() => {
            this.changeCurrentPage(totalPages);
          }}
        >
          <li>{totalPages}</li>
        </a>
  ...
  render() {
    return (
      <div className={this.props.theme + " pagination row justify-content-center"}>
        <ul>
          {this.showPrev()}
